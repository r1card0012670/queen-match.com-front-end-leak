    ...
    <div :class="[`d-time-plan-cast p-0`]">
        <v-col class="d-box-change-time p-0">
            <swiper :options="swiperOption" ref="swiperSlider" :id="renderKeyId">
                <swiper-slide v-for="(item, index) of renderTimePlan" :key="index" :data-id="item.id">
...
export default {
    ...
    computed: {
        ...
        renderTimePlan: function () {
            ...
            Array(24).fill(1).forEach((ele, index) => {
                ...
                results.push({id: `${txtIndex}00`, value: `${txtIndex}:00`})
                results.push({id: `${txtIndex}${distanceTime}`, value: `${txtIndex}:${distanceTime}`})
            });
            if (this.type === 'to') {
                results.push({id: `2359`, value: `23:59`})
            }
    ...
    methods: {
        ...
        getIndexElement: function (elementId, key) {
            let keyDefault = key || `${appUtility.dateFormat(new Date(), "HH")}00`;
