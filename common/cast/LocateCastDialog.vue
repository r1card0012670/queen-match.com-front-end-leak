<template>
    <div :class="[`d-dialog-locate-cast p-0`]" v-if="isShowDialog">
        <v-bottom-sheet v-if="isShowDialog" v-model="showLocateCastDialog" persistent scrollable
            ...
            <v-card :class="[`d-locate-cast`, 'd-border-radius-top-15']" :id="`dialog-locate-cast`" height="50vh">
                ...
                <v-card-text :id="'d-card-text-local-cast'" class="d-card-text-local-cast">
                    <v-radio-group v-model="vModelDataRadio" :class="`mt-0 pt-0`">
                        <v-radio v-for="(item, index) in locationItems" :key="index" selected :class="`form-check`"
                                 :label="`${item.value}`"
                                 :value="item"></v-radio>
                    </v-radio-group>
                </v-card-text>
...
export default {
    ...
    mounted: function () {
        if (this.value) {
            this.vModelDataRadio = this.value
        }
    },
    computed: {
        ...mapState({
    ...
    methods: {
        ...
        clickCallbackEventData: function () {
            //this.funcEmitData();
            // this.updateUser({user_id: this.user.id, residence: this.vModelDataRadio.id});
            this.$emit('change', this.vModelDataRadio);
        ...
        /*funcEmitData: function () {
            if (!this.vModelDataRadio) {
                return;
            }
            let locateItem = this.locationItems.filter(ele => ele.id === this.vModelDataRadio);
            if (locateItem) {
                this.$emit('change', locateItem[0]);
            }
        }*/
    },
    created() {
