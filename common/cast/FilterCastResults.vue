<template>
    <div :class="`app-content-box`" v-if="showDialogFilterCastResult">
        <v-row justify="center" :id="`d-filter-cast-results-wrapper`">
            <v-dialog v-model="showDialogFilterCastResult" :content-class="`d-dialog-show-fullscreen`" max-width="440"
                ...
                <v-card :class="`d-dialog-search-cast-for-guest-results`" :id="`dialog-search-casts-for-guest-results`">
                    <v-card-title :id="'d-card-title-filter-cast-results'"
                        ...
                        <div :class="`card-header-top`">
                            <button @click="clickFilterCastClose">
                    ...
                    <v-card-text :id="'d-card-text-filter-cast-results'" :class="`mb-60`">
                                    ...
                                    <div class="text-center">
                                        <template v-for="item in groupTags">
                                            <v-chip v-for="tag in item.tags" :key="tag.id"
                                                    v-if="getSelectedTagAttribute(tag)" close
                        ...
                        <div class="card-body d-list-content">
                            <div class="d-inline-flex d-list-show-all-users box-1 p-0 d-box-new-card">
                                <CardUserComponent v-for="(item, index) in searchResult" :key="index"
                                                   :itemData="item"
                                                   :type="'all-users'"
                                                   :imgAvatarDefault="item.avatar"/>

                                <div ref="infiniteScrollFilterCast"></div>
                            </div>
                            <div v-if="noResult"
                                ...
                                <span class="mt-7"> 検索条件に合うキャストがいません </span>
                            </div>
                        </div>
                        <v-sheet ref="form" class="box-position-footer">
                            <v-btn block large @click="clickFilterCastContinue" :class="`mt-7 btn-base`">
                                検索条件を再設定する
                            </v-btn>
                        </v-sheet>
                    </v-card-text>
...
export default {
    ...
    methods: {
        ...mapActions({
            getSearchCastResult: 'getSearchCastResult',
        }),
        ...mapMutations({
            setSearchCastConditions: 'setSearchCastConditions',
        }),
        clickFilterCastContinue: function (event) {
            this.resetInfinityScroll();
            this.$store.dispatch('showDialogFilterCastResultDispatch', {isShow: false, data: {}, reset: true});
        },
        clickFilterCastClose: function (event) {
            this.resetInfinityScroll();
            this.$store.dispatch('showDialogFilterCastResultDispatch', {isShow: false, data: {}, reset: true});
        },
        fetchListData: function () {
            this.$store.dispatch('getSearchCastResult', {...this.searchCastCondition, ...{page: this.page}}).then(response => {
        ...
        scrollTrigger: function () {
            ...
            if (!!self.$refs.infiniteScrollFilterCast) {
                this.observer.observe(self.$refs.infiniteScrollFilterCast);
            }
        },
        // infiniteHandler($state) {
        //     this.$store.dispatch('getSearchCastResult', {...this.searchCastCondition, ...{page: this.page}})
        //         .then(response => {
        //             this.searchResult.push(...response.data.data);
        //             // console.log(this.searchResult);
        //             if (!this.searchResult.length && this.firstLoad) {
        ...
        getSelectedTagAttribute(tag) {
            return this.searchCastCondition.hashTags.includes(tag.id);
        },
        removeTag(tag) {
            const tags = [...this.searchCastCondition.hashTags];
            const index = tags.indexOf(tag.id);
            tags.splice(index, 1);
            this.$store.commit('setSearchCastConditions', {...this.searchCastCondition, hashTags: tags});
    ...
    computed: {
        ...mapState({
            ...
            filterCastResults: state => state.cast.filterCastResults,
            showDialogFilterCastResult: state => state.cast.showDialogFilterCastResult,
        }),
        ...mapGetters({
            showDialogFilterCast: "showDialogFilterCast",
            showDialogFilterCastResult: "showDialogFilterCastResult",
            filterCast: "filterCast",
            filterCastResults: "filterCastResults",
            searchCastCondition: 'searchCastCondition',
            groupTags: 'groupTags',
        }),
        constant: function () {
